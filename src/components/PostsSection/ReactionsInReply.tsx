...
import { fetchReactionsInReply } from 'src/services/reaction-in-reply';
...
function TabPanel(props: TabPanelProps) {
  ...
  const { value, index, reactionType, commentId, postId, replyId, counts } =
    props;
  ...
  const loadMore = useCallback(async () => {
    ...
    const response = await fetchReactionsInReply({
      replyId,
      reactionType,
      lastReactionId: reactions[reactions.length - 1]?._id,
      limit,
    });
    const { reactions: newReactions, reply } = response.data.data;
    dispatch(updateReply(replyId, commentId, postId, reply));
    setReactions((prev) => [...prev, ...newReactions]);
    setIsLoading(false);
  }, [limit, commentId, replyId, reactionType, dispatch, postId, reactions]);
...
interface ReactionsInReplyProps {
  postId: string;
  commentId: string;
  replyId: string;
  counts: {
...
const Reactions: React.FC<ReactionsInReplyProps> = React.forwardRef(
  ({ postId, counts, commentId, replyId }, ref) => {
